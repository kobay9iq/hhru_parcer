...
def ExtractAllVacancies(vacancy="python"):
    ...
    for page in range(last_page):
        try:
            ...
            print(f"Не удалось загрузить страницу под номером {page}")
            continue
...
def ExtractVacancyInfo(html):
        tittle = html.find("a").text.replace("\\xa0", " ").strip()
        
        company = html.find("div", {"class": 
                "vacancy-serp-item__meta-info-company"}).find("a")\
                    .text.replace("\\xa0", " ").strip()
        
        city = html.find("div", {"data-qa":
                "vacancy-serp__vacancy-address"}).text.\
                    replace("\\xa0", " ").strip()
        if "," in city: city = city.split(",")[0]
        
        experience = html.find("div", {"data-qa":
                "vacancy-serp__vacancy-work-experience"}).text.\
                    replace("\\xa0", " ").strip()
        
        salary = html.find("div", {"data-qa":
                "vacancy-serp__vacancy-compensation"})
        if salary is not None:
            salary = salary.text.replace("\\xa0", " ").strip()
        else:
            salary = "Не указано"
